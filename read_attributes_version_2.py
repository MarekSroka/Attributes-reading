...
def process_xml_file(file_path):
    ...
    if 'valueType' in df.columns:
        columns = [col for col in df.columns if col != 'valueType'] + ['valueType']
        df = df[columns]
...
output_file = 'XML_Attributes.xlsx'
with pd.ExcelWriter(output_file) as writer:
    for object_type in sorted(dfs.keys()):
        combined_df = pd.concat(dfs[object_type], ignore_index=True)
